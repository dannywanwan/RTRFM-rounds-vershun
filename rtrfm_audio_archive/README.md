...
The app checks once every Sunday at 3:00am Australia/Perth time by default and does not require a QNAP mount. The `sunday_hour` option can change the hour. It also installs the dashboard card automatically at `/config/www/rtrfm-episode-card.js` when the app starts.
