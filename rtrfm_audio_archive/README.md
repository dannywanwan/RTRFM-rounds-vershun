...
The app runs one check when it starts, then checks again every Sunday at 3:00am Australia/Perth time by default. Restarting the app provides an on-demand refresh. It does not require a QNAP mount and downloads The Rounds, Jamdown Vershun, and Trainwreck, keeping each show in its own folder under `/media/rtrfm/`. The `sunday_hour` option can change the hour. It also installs the dashboard card automatically at `/config/www/rtrfm-episode-card.js` when the app starts.
